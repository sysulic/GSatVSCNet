...
def list_to_nested_tuple(input_list):
    if isinstance(input_list, list):
        return tuple(list_to_nested_tuple(item) for item in input_list)
    else:
        return input_list

def check(ltl,trace,vocab):
    '''
    ltl必须是二叉树元组（op，左子树，右子树），子树如果是原子命题是字符串，否则为二叉树元组。
    '''
    
    t,loop_start=trace
    trace={'name':'t','trace':t}
    # formula=ltl2turple(ltl)
    # formula =list_to_nested_tuple(ltl)
    formula = ltl
    if printproof:
...
if __name__ == '__main__':
    ...
    ltl='((!(F((!(p24)) | (F(X(F((p21) & (F(!(F(G(p42)))))))))))) | (G((p9) | (X(p56))))) & (F((X(!(p96))) | (G(F(!(X(G(F(p79))))))))) & (F(G((X(X((X((F((!(p17)) U ((X(p13)) | ((X(p91)) & (!(G(p89))))))) & (G((!((p95) & ((p37) | (F(p77))))) U ((p93) & (!(F(p83)))))) & (!(((X(p93)) U ((p76) | ((p9) & (p33)))) | ((F(p5)) U (p60)) | (!((p83) & ((p31) U (p71)))))))) U ((F(G((!((p53) & (p92))) U (!(p71))))) U (G(F(!(G((X((X(p77)) U (X(X(p53))))) U ((p52) | (p67))))))))))) & (X(G((p82) U (p77)))))))'
    trace=([['p76', 'p71', 'p21', 'p9', 'p13', 'p95', 'p53', 'p42', 'p92'], ['p71', 'p9', 'p13', 'p52', 'p89', 'p42', 'p77'], ['p76', 'p96', 'p82', 'p13', 'p89', 'p95', 'p93', 'p42'], ['p96', 'p71', 'p9', 'p52', 'p56', 'p89', 'p42', 'p77'], ['p76', 'p96', 'p82', 'p13', 'p89', 'p95', 'p93', 'p42']],2)
    vocab={'p24': 109, 'p21': 30, 'p42': 112, 'p9': 31, 'p56': 71, 'p96': 17, 'p79': 96, 'p17': 90, 'p13': 45, 'p91': 35, 'p89': 81, 'p95': 84, 'p37': 98, 'p77': 123, 'p93': 111, 'p83': 63, 'p76': 6, 'p33': 77, 'p5': 43, 'p60': 69, 'p31': 125, 'p71': 22, 'p53': 92, 'p92': 127, 'p52': 59, 'p67': 91, 'p82': 39}
    value=check(ltl2turple(ltl),trace,vocab.keys())
    print('value',value)
